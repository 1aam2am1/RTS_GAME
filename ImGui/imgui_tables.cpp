...
static ImGuiTableColumnFlags TableFixColumnFlags(ImGuiTable *table, ImGuiTableColumnFlags flags) {
    ...
    if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
        ...
        if (table->Flags & ImGuiTableFlags_ColumnsWidthFixed)
            flags |= ((table->Flags & ImGuiTableFlags_Resizable) && !(flags & ImGuiTableColumnFlags_NoResize))
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        column->Flags =
                TableFixColumnFlags(table, column->FlagsIn) | (column->Flags & ImGuiTableColumnFlags_StatusMask_);
        ...
        if (!column->IsPreserveWidthAuto) {
            const float content_width_body =
                    (float) ImMax(column->ContentMaxXFrozen, column->ContentMaxXUnfrozen) - column->WorkMinX;
    ...
    const float width_avail_for_stretched_columns = mixed_same_widths ? 0.0f : width_avail - width_spacings -
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
            ...
            if (!mixed_same_widths) {
                ...
                column->WidthRequest = IM_FLOOR(
                        ImMax(width_avail_for_stretched_columns * weight_ratio, min_column_width) + 0.01f);
    ...
    if (width_remaining_for_stretched_columns >= 1.0f && !(table->Flags & ImGuiTableFlags_PreciseWidths))
        for (int order_n = table->ColumnsCount - 1;
             sum_weights_stretched > 0.0f && width_remaining_for_stretched_columns >= 1.0f && order_n >= 0; order_n--) {
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
        ...
        if (table->Flags & ImGuiTableFlags_ScrollX) {
            ...
            if (order_n < table->FreezeColumnsRequest) {
                max_width =
                        (table->InnerClipRect.Max.x - (table->FreezeColumnsRequest - order_n) * min_column_distance) -
                        offset_x;
...
void ImGui::EndTable() {
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX) {
        ...
        outer_window->DC.CursorMaxPos.x = ImMax(backup_outer_max_pos_x,
...
ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id) {
    ...
    IM_ASSERT(table->DeclColumnsCount >= 0 && table->DeclColumnsCount < table->ColumnsCount &&
...
void ImGui::TableSetupDrawChannels(ImGuiTable *table) {
    ...
    table->Bg1DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx) ((table->FreezeRowsCount > 0) ? 2 + channels_for_row
...
void ImGui::TableDrawBorders(ImGuiTable *table) {
    ...
    const float draw_y2_head = table->IsUsingHeaders ? (
            (table->FreezeRowsCount >= 1 ? table->OuterRect.Min.y : table->WorkRect.Min.y) + table->LastFirstRowHeight)
                                                     : draw_y1;
...
void ImGui::TableFixColumnSortDirection(ImGuiTableColumn *column) {
    ...
    if (column->SortDirection == ImGuiSortDirection_None)
        column->SortDirection = (column->Flags & ImGuiTableColumnFlags_PreferSortDescending)
                                ? (ImS8) ImGuiSortDirection_Descending : (ImS8) (ImGuiSortDirection_Ascending);
        ...
        if (pressed && table->ReorderColumn != column_n) {
            ...
            if (column->SortOrder == -1)
                sort_direction = (column->Flags & ImGuiTableColumnFlags_PreferSortDescending)
                                 ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending;
            else
                sort_direction = (column->SortDirection == ImGuiSortDirection_Ascending) ? ImGuiSortDirection_Descending
...
void ImGui::DebugNodeTable(ImGuiTable *table) {
    ...
    BulletText("OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f)", table->OuterRect.Min.x, table->OuterRect.Min.y,
               table->OuterRect.GetWidth(), table->OuterRect.GetHeight());
    BulletText("ColumnsWidth: %.1f, AutoFitWidth: %.1f, InnerWidth: %.1f%s", table->ColumnsTotalWidth,
